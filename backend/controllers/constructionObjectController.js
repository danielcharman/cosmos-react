...
const updatePlanetBuilding = asyncHandler(async (req, res) => {
	...
	//calculate costs
	const resourceCosts = await getObjectResourceCosts(planetObject.object, req.body.amount)
	...
	const updatedPlanetBuilding = await PlanetObject.findByIdAndUpdate(
		req.params.planetObjectId,
		{
			active: false
		}
	)
...
const getPlanetTechnologies = asyncHandler(async (req, res) => {
	...
	const technologies = await Promise.all(tempObjects.map(async (object) => {
		let planetObject = await PlanetObject.findOne({
			planet: req.params.planetId,
			object: object._id
		})

		console.log('WOOKIESSSSS',planetObject,{
			planet: req.params.planetId,
			object: object._id
		})
...
const updatePlanetTechnology = asyncHandler(async (req, res) => {
	...
	const updatedPlanetObject = await PlanetObject.findByIdAndUpdate(
		req.params.planetObjectId,
		{
			active: false
		}
